local requestsDisabled = getgenv and getgenv().DISABLE_SPUNK_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.671"
local SpunkFolder = "Spunk"
local ConfigurationFolder = SpunkFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		spunkOpen = {Type = 'bind', Value = 'K', Name = 'Spunk Keybind'},
	},
	System = {
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(SpunkFolder) then
				if isfile and isfile(SpunkFolder..'/settings'..ConfigurationExtension) then
					file = readfile(SpunkFolder..'/settings'..ConfigurationExtension)
				end
			end
			if useStudio then
				file = [[
		{"General":{"spunkOpen":{"Value":"K","Type":"bind","Name":"Spunk Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Spunk Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('We ran into an issue accessing configuration saving capability.')
		end
	end
end
...
local Spunk = useStudio and script.Parent:FindFirstChild('Spunk') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Spunk:FindFirstChild('Build') and Spunk.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Spunk | Build Mismatch')
		print('Spunk may encounter issues as you are running an incompatible interface version ('.. ((Spunk:FindFirstChild('Build') and Spunk.Build.Value) or 'No Build') ..').\n\nThis version of Spunk is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Spunk = Spunk, useStudio and script.Parent:FindFirstChild('Spunk') or game:GetObjects("rbxassetid://10804731440")[1]
...
Spunk.Enabled = false
Spunk.Parent = game.Players.LocalPlayer.PlayerGui

local minSize = Vector2.new(1024, 768)
local useMobileSizing
if Spunk.AbsoluteSize.X < minSize.X and Spunk.AbsoluteSize.Y < minSize.Y then
	useMobileSizing = true
end
if UserInputService.TouchEnabled then
	useMobilePrompt = true
end
local Main = Spunk.Main
local MPrompt = Spunk:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Spunk:FindFirstChild('Drag')
...
local Notifications = Spunk.Notifications
local SelectedTheme = SpunkLibrary.Theme.Default
local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = SpunkLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Spunk.Main.BackgroundColor3 = SelectedTheme.Background
	Spunk.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Spunk.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Spunk.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Spunk.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Spunk.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Spunk.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Spunk.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Spunk.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Spunk:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Spunk had an issue decoding the configuration file, please try delete the file and reopen Spunk.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(SpunkLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Spunk | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--SpunkLibrary:Notify({Title = "Spunk Flags", Content = "Spunk was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(SpunkLibrary.Flags) do
...
function SpunkLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Spunk | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			SpunkLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Spunk'.", Duration = 7, Image = 4400697855})
		else
			SpunkLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.spunkOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(SpunkFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Spunk Settings', 0, true)

	if TabList['Spunk Settings'] then
		TabList['Spunk Settings'].LayoutOrder = 1000
	end

	if Elements['Spunk Settings'] then
		Elements['Spunk Settings'].LayoutOrder = 1000
	end
	for categoryName, settingCategory in pairs(settingsTable) do
...
function SpunkLibrary:CreateWindow(Settings)
	if Spunk:FindFirstChild('Loading') then
        Spunk.Loading:Destroy()
	end

	if getgenv then getgenv().spunkCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				SpunkLibrary:Notify({Title = 'Build Mismatch', Content = 'Spunk may encounter issues as you are running an incompatible interface version ('.. ((Spunk:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(SpunkFolder) then
		makefolder(SpunkFolder)
	end
	...
	if Settings.Theme then
		...
		if not success then
			...
			if not success then
				warn('CRITICAL ERROR - NO THEME')
				print(result2)
			end
			warn('Rendering Issue, No Theme.')
			print(result)
		end
	end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(SpunkFolder.."/Discord Invites") then
			makefolder(SpunkFolder.."/Discord Invites")
		end

		if isfile and not isfile(SpunkFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(SpunkFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Spunk RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(SpunkFolder.."/Key System") then
			makefolder(SpunkFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Spunk | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Spunk specific development.')
				end
			end
		end
		...
		if isfile and isfile(SpunkFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(SpunkFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
		...
		if not Passthrough then
			...
			KeyUI.Parent = game.Players.LocalPlayer.PlayerGui
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SpunkFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						SpunkLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Spunk | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Spunk specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= SpunkLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SpunkLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Spunk.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Spunk | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Spunk specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					SpunkLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Spunk | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check discord.gg/assetgg for help with Spunk specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SpunkLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Spunk | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check discord.gg/assetgg for help with Spunk specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SpunkLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= SpunkLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Spunk | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check discord.gg/assetgg for help with Spunk specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						SpunkLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Spunk | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check discord.gg/assetgg for help with Spunk specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SpunkLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
			Spunk.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				if SelectedTheme ~= SpunkLibrary.Theme.Default then
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			SpunkLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			SpunkLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	local success, result = pcall(function()
		createSettings(Window)
	end)
	if not success then warn('Spunk had an issue creating settings.') end
	return Window
end
...
function SpunkLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end
function SpunkLibrary:IsVisible(): boolean
	return not Hidden
end
local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function SpunkLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Spunk Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.spunkOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function SpunkLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				SpunkLibrary:Notify({Title = "Spunk Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			SpunkLibrary:Notify({Title = "Spunk Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Spunk Configurations Error | '..tostring(result))
			SpunkLibrary:Notify({Title = "Spunk Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	SpunkLibrary.LoadConfiguration()
